package io.github.tivj.teemutweaks.utils;
...
public class BytecodeHelper {
    public static void printEmAll(InsnList instructions) {
        TTMain.LOGGER.info("BytecodeDump:"+"------- Method dump start --------");
        for (ListIterator<AbstractInsnNode> it = instructions.iterator(); it.hasNext();) {
            ...
            if (instruction instanceof MethodInsnNode)          TTMain.LOGGER.info("BytecodeDump:"+opcodeToString(instruction.getOpcode())+" "+methodToMCP((MethodInsnNode) instruction));
            else if (instruction instanceof FieldInsnNode)      TTMain.LOGGER.info("BytecodeDump:"+opcodeToString(instruction.getOpcode())+" "+fieldToMCP((FieldInsnNode) instruction));
            else if (instruction instanceof VarInsnNode)        TTMain.LOGGER.info("BytecodeDump:"+opcodeToString(instruction.getOpcode())+" "+((VarInsnNode) instruction).var);
            else if (instruction instanceof TypeInsnNode)       TTMain.LOGGER.info("BytecodeDump:"+opcodeToString(instruction.getOpcode())+" "+FMLDeobfuscatingRemapper.INSTANCE.mapType(((TypeInsnNode) instruction).desc));
            else if (instruction instanceof LdcInsnNode)        TTMain.LOGGER.info("BytecodeDump:LDC " + ((LdcInsnNode) instruction).cst);
            else if (instruction instanceof LabelNode)          TTMain.LOGGER.info("BytecodeDump:LABELNODE");
            else if (instruction instanceof LineNumberNode)     {TTMain.LOGGER.info("BytecodeDump:\n"); TTMain.LOGGER.info("BytecodeDump:Line: "+((LineNumberNode) instruction).line);}

            else TTMain.LOGGER.info("BytecodeDump:"+opcodeToString(instruction.getOpcode()));
        }
        TTMain.LOGGER.info("BytecodeDump:------- Method dump end --------\n");
    }
